...
public class Board_Controller : MonoBehaviour
{
    ...
    private void SpawnGem(Vector2Int pos, Gem gemToSpwan)
    {
        if (Random.Range(0f, 100f) < bombChance)
        {
            gemToSpwan = bomb;//bomba bonusu
        }

        Gem gem = Instantiate(gemToSpwan, new Vector3(pos.x, pos.y + height, 0f), Quaternion.identity);
